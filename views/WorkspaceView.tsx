...
import { 
  Mail, Calendar, HardDrive, Video, FileText, Table, Presentation, 
  Plus, Search, Star, Trash2, Mic, Camera, PhoneOff, Settings, 
  MoreVertical, ChevronLeft, ChevronRight, Grid, List, Download, Share2, 
  Inbox, Send, File, Clock, Users, Layout, Video as VideoIcon,
  X, Paperclip, Check, MicOff, CameraOff, MonitorUp, MessageSquare,
  Bold, Italic, Underline, AlignLeft, AlignCenter, AlignRight, List as ListIcon,
  Type, UserPlus, Smile, Bell, Calculator, AlertTriangle, Folder, ArrowUp, Home
} from 'lucide-react';
...
const EmailApp = ({ onComposeReply, searchQuery }: { onComposeReply?: (email: WorkspaceEmail) => void, searchQuery: string }) => {
  ...
  return (
    <div className="flex h-full bg-white dark:bg-gray-900 rounded-3xl overflow-hidden border border-gray-200 dark:border-gray-700 relative">
          ...
          <div className="absolute inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-white dark:bg-gray-900 w-full max-w-lg rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
                  <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50">
                      ...
                      <button onClick={() => setComposing(false)}><X size={20} className="text-gray-400 hover:text-red-500"/></button>
                  </div>
                  <div className="p-4 space-y-3">
                      <input 
                        ...
                        className="w-full p-2 bg-transparent border-b border-gray-100 dark:border-gray-800 outline-none text-sm font-bold text-gray-900 dark:text-white"
                      />
                      <input 
                        placeholder="Assunto" 
                      ...
                      <textarea 
                        ...
                        className="w-full h-40 p-2 bg-transparent outline-none text-sm resize-none text-gray-900 dark:text-white"
                      ></textarea>
                  </div>
                  <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex justify-end gap-2">
                      <button onClick={() => setComposing(false)} className="px-4 py-2 text-gray-500 font-bold text-xs hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg">Cancelar</button>
                      <button onClick={handleSend} className="px-6 py-2 bg-blue-600 text-white font-bold text-xs rounded-lg hover:bg-blue-700 flex items-center gap-2">
                          <Send size={14}/> Enviar
                      </button>
                  </div>
              </div>
      ...
      <div className={`w-full md:w-1/3 border-r border-gray-100 dark:border-gray-800 flex flex-col ${selectedEmail ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-4 space-y-2 border-b border-gray-100 dark:border-gray-800">
            <button onClick={() => { setNewEmail({to:'',subject:'',body:''}); setComposing(true); }} className="w-full py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/30 font-bold text-sm flex items-center justify-center gap-2">
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            {filteredEmails.map(email => (
                <div 
                    ...
                    className={`p-4 border-b border-gray-50 dark:border-gray-800 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-colors ${selectedEmail?.id === email.id ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}
                >
      ...
      {/* Email Detail */}
      <div className={`flex-1 flex flex-col bg-gray-50 dark:bg-gray-800/50 ${!selectedEmail ? 'hidden md:flex items-center justify-center' : 'flex'}`}>
          {selectedEmail ? (
              <>
                <div className="p-6 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex justify-between items-start">
                    <div>
                        <button onClick={() => setSelectedEmail(null)} className="md:hidden mb-4 text-blue-500 flex items-center gap-1 text-xs font-bold"><ChevronLeft size={14}/> Voltar</button>
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2 leading-tight">{selectedEmail.subject}</h2>
                        <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-600 flex items-center justify-center font-bold text-xs">{selectedEmail.from[0]}</div>
                            <div>
                    ...
                    <div className="flex gap-2">
                        {activeFolder === 'trash' ? (
                            <button onClick={() => handlePermanentDelete(selectedEmail.id)} className="p-2 text-red-500 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-full" title="Excluir Permanentemente"><X size={18}/></button>
                        ) : (
                            <button onClick={() => handleMoveToTrash(selectedEmail.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full"><Trash2 size={18}/></button>
                        )}
                    </div>
                </div>
                <div className="p-8 flex-1 overflow-y-auto font-serif text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-wrap">
                ...
                <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex gap-2">
                    {activeFolder !== 'trash' && (
                        <button onClick={handleReply} className="flex-1 py-3 bg-blue-600 rounded-xl text-sm font-bold text-white hover:bg-blue-700 flex items-center justify-center gap-2"><Send size={16}/> Responder</button>
                    )}
                </div>
              </>
          ) : (
...
// 2. CALENDAR APP INTERACTIVE (No changes needed - already solid)
const CalendarApp = ({ searchQuery }: { searchQuery: string }) => {
    ...
    return (
        <div className="h-full flex flex-col bg-white dark:bg-gray-900 rounded-3xl overflow-hidden border border-gray-200 dark:border-gray-700 relative">
            ...
            {selectedEvent && (
                <div className="absolute inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in" onClick={() => setSelectedEvent(null)}>
                    <div className="bg-white dark:bg-gray-900 w-full max-w-sm rounded-[2rem] p-6 shadow-2xl relative border border-gray-100 dark:border-gray-700" onClick={e => e.stopPropagation()}>
                        <button onClick={() => setSelectedEvent(null)} className="absolute top-4 right-4 p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full"><X size={18}/></button>
                        ...
                        <button onClick={() => { setEvents(events.filter(e => e.id !== selectedEvent.id)); setSelectedEvent(null); }} className="w-full py-3 border border-red-200 text-red-500 font-bold rounded-xl hover:bg-red-50 transition-colors text-xs flex items-center justify-center gap-2">
            ...
            <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                    <button className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full"><ChevronLeft size={20}/></button>
                    <button className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full"><ChevronRight size={20}/></button>
                    <button onClick={() => setCurrentDay(new Date().getDate())} className="px-4 py-2 bg-blue-600 text-white rounded-full font-bold text-sm">Hoje</button>
                </div>
            </div>
            
            <div className="flex-1 grid grid-cols-7 grid-rows-5 gap-px bg-gray-100 dark:bg-gray-800">
                {Array.from({length: 31}, (_, i) => i + 1).map(day => {
                    ...
                    return (
                        <div 
                            ...
                            className={`bg-white dark:bg-gray-900 min-h-[80px] p-2 relative group hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors cursor-pointer ${isSelected ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''} ${isSearchMatch ? 'bg-yellow-50 dark:bg-yellow-900/10' : ''}`}
                        >
                            <span className={`text-xs font-bold ${isSelected ? 'bg-blue-600 text-white w-6 h-6 flex items-center justify-center rounded-full shadow-md' : 'text-gray-500'}`}>{day}</span>
                            <div className="mt-1 space-y-1">
                                {visibleEvents.map(ev => (
                                    <div 
                                        key={ev.id} 
                                        onClick={(e) => { e.stopPropagation(); setSelectedEvent(ev); }}
                                        className={`text-[9px] px-1 py-0.5 rounded truncate font-bold hover:opacity-80 ${
                            ...
                            {isSelected && !searchQuery && (
                                <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10" onClick={e => e.stopPropagation()}>
                                    <div className="bg-white dark:bg-gray-800 p-2 rounded-xl flex gap-1 shadow-xl">
                                        ...
                                        />
                                        <button onClick={handleAddEvent} className="text-blue-600"><Plus size={14}/></button>
                                    </div>
                                </div>
                            )}
                        </div>
...
const DriveApp: React.FC<DriveAppProps> = ({ onOpenFile, searchQuery }) => {
    ...
    return (
        <div className="h-full flex flex-col bg-white dark:bg-gray-900 rounded-3xl overflow-hidden border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-400 mt-1">
                        <button onClick={() => setCurrentFolderId(null)} className="hover:text-blue-500 cursor-pointer flex items-center gap-1">
                            <Home size={10}/> Início
                        </button>
                        {currentFolderId && (
                            <>
                ...
                <button 
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl font-bold text-sm shadow-lg hover:bg-blue-700 transition-colors"
                >
                    <Plus size={16}/> Upload
                </button>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 overflow-y-auto pb-10">
                ...
                {filteredFiles.map(file => (
                    <div 
                        ...
                        className="group p-4 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-blue-400 hover:shadow-lg transition-all cursor-pointer bg-gray-50 dark:bg-gray-800/50 flex flex-col items-center text-center relative"
                    >
                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                            <button onClick={(e) => { e.stopPropagation(); handleDelete(file.id); }} className="p-1 hover:bg-red-100 hover:text-red-500 rounded text-gray-400"><Trash2 size={12}/></button>
                        </div>
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-3 ${
                            file.type === 'doc' ? 'bg-blue-100 text-blue-600' :
                            file.type === 'sheet' ? 'bg-green-100 text-green-600' :
                            file.type === 'slide' ? 'bg-orange-100 text-orange-600' :
                            file.type === 'pdf' ? 'bg-red-100 text-red-600' :
                            'bg-yellow-100 text-yellow-600'
                        }`}>
                            {file.type === 'doc' && <FileText size={24}/>}
                            {file.type === 'sheet' && <Table size={24}/>}
                            {file.type === 'slide' && <Presentation size={24}/>}
                            {file.type === 'pdf' && <FileText size={24}/>}
                            {file.type === 'folder' && <Folder size={24}/>}
                        </div>
...
const MeetApp = () => {
    ...
    return (
        <div className="h-full flex flex-col bg-black rounded-3xl overflow-hidden relative">
                ...
                {(showChat || showParticipants) && (
                    <div className="absolute right-2 top-2 bottom-2 w-72 bg-gray-900/95 backdrop-blur-xl border border-white/10 rounded-2xl flex flex-col z-20 animate-in slide-in-from-right">
                        <div className="flex justify-between items-center p-4 border-b border-white/10">
                            ...
                            <button onClick={() => { setShowChat(false); setShowParticipants(false); }}><X size={16} className="text-gray-400 hover:text-white"/></button>
                        </div>
            ...
            <div className="h-20 bg-gray-900/80 backdrop-blur-md flex items-center justify-center gap-4">
                <button onClick={() => setMicOn(!micOn)} className={`p-4 rounded-full transition-all duration-300 ${micOn ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-red-500/20 text-red-500 hover:bg-red-500 hover:text-white'}`}>{micOn ? <Mic size={20}/> : <MicOff size={20}/>}</button>
                <button onClick={() => setCamOn(!camOn)} className={`p-4 rounded-full transition-all duration-300 ${camOn ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-red-500/20 text-red-500 hover:bg-red-500 hover:text-white'}`}>{camOn ? <Camera size={20}/> : <CameraOff size={20}/>}</button>
                <button className="p-4 rounded-full bg-red-600 text-white hover:bg-red-700 transition-colors shadow-lg scale-110 mx-2"><PhoneOff size={24}/></button>
                <button onClick={() => { setShowChat(!showChat); setShowParticipants(false); }} className={`p-4 rounded-full transition-all duration-300 ${showChat ? 'bg-blue-600 text-white' : 'bg-gray-700 text-white hover:bg-gray-600'}`}><MessageSquare size={20}/></button>
                <button onClick={() => { setShowParticipants(!showParticipants); setShowChat(false); }} className={`p-4 rounded-full transition-all duration-300 ${showParticipants ? 'bg-blue-600 text-white' : 'bg-gray-700 text-white hover:bg-gray-600'}`}><Users size={20}/></button>
            </div>
        </div>
    );
};

// 5. EDITOR APP (DOCS/SHEETS) - UPDATED WITH FORMULA LOGIC
        ...
        <div className="h-full flex flex-col bg-white dark:bg-gray-900 rounded-3xl overflow-hidden border border-gray-200 dark:border-gray-700">
            ...
            <div className="flex flex-col border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
                <div className="p-3 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg text-white ${type === 'doc' ? 'bg-blue-600' : 'bg-green-600'}`}>
                            {type === 'doc' ? <FileText size={20}/> : <Table size={20}/>}
                        </div>
                        <div>
                            <input 
                                value={fileName}
                            ...
                            <div className="flex gap-2 text-[10px] text-gray-500">
                                ...
                                <span className="hover:text-blue-500 cursor-pointer">Inserir</span>
                            </div>
                        </div>
                    </div>
                    <button className="px-4 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-blue-200 transition-colors">
                ...
                <div className="px-3 pb-2 flex gap-1 overflow-x-auto scrollbar-hide">
                    <button onClick={() => execCmd('bold')} className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-gray-600 dark:text-gray-300"><Bold size={14}/></button>
                    <button onClick={() => execCmd('italic')} className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-gray-600 dark:text-gray-300"><Italic size={14}/></button>
                    <button onClick={() => execCmd('underline')} className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-gray-600 dark:text-gray-300"><Underline size={14}/></button>
                    <div className="w-px bg-gray-300 dark:bg-gray-700 mx-1 h-4 self-center"></div>
                    <button onClick={() => execCmd('justifyLeft')} className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-gray-600 dark:text-gray-300"><AlignLeft size={14}/></button>
                    <button onClick={() => execCmd('justifyCenter')} className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-gray-600 dark:text-gray-300"><AlignCenter size={14}/></button>
                    <button onClick={() => execCmd('justifyRight')} className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-gray-600 dark:text-gray-300"><AlignRight size={14}/></button>
                    <div className="w-px bg-gray-300 dark:bg-gray-700 mx-1 h-4 self-center"></div>
                    <button className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-gray-600 dark:text-gray-300"><ListIcon size={14}/></button>
                    <button className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-gray-600 dark:text-gray-300"><Type size={14}/></button>
                </div>
            </div>
                        ...
                        <div className="flex flex-col">
                            <div className="grid grid-cols-4 gap-0 border-t border-l border-gray-300 dark:border-gray-700 font-sans text-xs select-none relative">
                                {['CATEGORIA', 'VALOR (R$)', 'DATA', 'STATUS'].map(h => (
                                    <div key={h} className="p-2 border-r border-b border-gray-300 dark:border-gray-700 font-bold bg-gray-100 dark:bg-gray-800 flex items-center justify-center">{h}</div>
                                ))}
                                        ...
                                        return (
                                            <div key={`${rowIndex}-${colIndex}`} className={`border-r border-b border-gray-300 dark:border-gray-700 p-0 relative group/cell ${isTotal ? 'bg-yellow-50 dark:bg-yellow-900/10 font-bold' : ''}`}>
                                                <input 
                                                    ...
                                                    className={`w-full h-full p-2 bg-transparent outline-none focus:bg-blue-50 dark:focus:bg-blue-900/20 text-center ${isTotal ? 'font-black text-gray-900 dark:text-white' : ''}`}
                                                />
                                                {!isTotal && colIndex === 0 && (
                                                    <button 
                                                        onClick={() => handleDeleteRow(rowIndex)}
                                                        className="absolute -left-6 top-1/2 -translate-y-1/2 opacity-0 group-hover/cell:opacity-100 p-1 text-red-400 hover:text-red-600 transition-opacity"
                                                        title="Remover Linha"
                                                    >
                                                        <Trash2 size={12} />
                                                    </button>
                            ...
                            <button 
                                ...
                                className="mt-4 self-start flex items-center gap-2 px-4 py-2 text-xs font-bold text-green-600 bg-green-50 dark:bg-green-900/20 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/40 transition-colors"
                            >
...
const WorkspaceView: React.FC = () => {
    ...
    return (
        <div className="w-full h-full bg-[#F3F4F6] dark:bg-black font-sans flex flex-col animate-in fade-in duration-500">
            ...
            <div className="p-4 md:p-6 pb-2 shrink-0">
                <div className="flex flex-col xl:flex-row justify-between items-center mb-6 gap-6">
                    ...
                    <div className="flex-1 w-full flex items-center justify-between xl:justify-start gap-8">
                        <div>
                            <h1 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight flex items-center gap-2">
                                <Layout className="text-green-600"/> Gabinete Digital
                            </h1>
                        ...
                        <div className="hidden md:flex flex-1 max-w-md relative group">
                            ...
                            <input 
                                ...
                                className="w-full pl-10 pr-4 py-3 bg-white dark:bg-gray-900 rounded-2xl text-sm font-medium shadow-sm border border-transparent focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none"
                            />
                        </div>
                        ...
                        <div className="relative">
                            <button 
                                ...
                                className="p-3 bg-white dark:bg-gray-900 rounded-full hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors relative shadow-sm"
                            >
                            ...
                            {showNotifications && (
                                <div className="absolute top-full right-0 mt-2 w-80 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden z-50 animate-in slide-in-from-top-2 fade-in">
                                    <div className="p-4 border-b border-gray-100 dark:border-gray-800 font-bold text-sm flex justify-between">
                                    ...
                                    <div className="max-h-64 overflow-y-auto">
                                        {[
                                            ...
                                            { icon: AlertTriangle, color: 'text-yellow-500', title: 'Votação Urgente Iniciada', time: '15 min' }
                                        ].map((n, i) => (
                                            <div key={i} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors cursor-pointer border-b border-gray-50 dark:border-gray-800 last:border-0 flex gap-3">
                                    ...
                                    <button className="w-full p-2 text-center text-[10px] font-bold text-blue-500 hover:bg-gray-50 dark:hover:bg-gray-800 uppercase">Ver Todas</button>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    {/* App Dock */}
                    <div className="flex gap-2 mt-4 xl:mt-0 bg-gray-100 dark:bg-gray-900 p-1.5 rounded-[2rem] shadow-inner overflow-x-auto max-w-full shrink-0">
