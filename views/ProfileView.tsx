...
const getStatusColor = (status: string) => {
    switch(status) {
        case 'Aprovado': return 'bg-blue-100 text-blue-900 border-blue-200 dark:bg-blue-900/40 dark:text-blue-200 dark:border-blue-800'; 
...
const TimelineIcon = ({ type }: { type: string }) => {
    switch(type) {
        case 'voto': return <div className="bg-blue-100 text-blue-900 p-2 rounded-full shadow-lg shadow-blue-500/20 z-10 border-2 border-white dark:border-gray-800"><Vote size={16}/></div>;
        case 'despesa': return <div className="bg-gray-100 text-gray-900 p-2 rounded-full shadow-lg shadow-gray-500/20 z-10 border-2 border-white dark:border-gray-800"><Banknote size={16}/></div>;
        case 'discurso': return <div className="bg-blue-50 text-blue-600 p-2 rounded-full shadow-lg shadow-blue-500/20 z-10 border-2 border-white dark:border-gray-800"><Mic2 size={16}/></div>;
        ...
        case 'evento': return <div className="bg-blue-100 text-blue-600 p-2 rounded-full shadow-lg shadow-blue-500/20 z-10 border-2 border-white dark:border-gray-800"><Presentation size={16}/></div>;
...
const ReceiptModal = ({ expense, politician, onClose }: { expense: TimelineItem, politician: Politician, onClose: () => void }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in" onClick={onClose}>
            <div className="flex flex-col items-center gap-4 animate-in zoom-in-95 duration-300 w-full max-w-sm" onClick={e => e.stopPropagation()}>
                ...
                <div id="viral-receipt" className="w-[340px] bg-[#fffdf0] text-black font-mono p-6 shadow-2xl relative rotate-1 overflow-hidden">
                    ...
                    {/* Carimbo Animado */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 border-[6px] border-yellow-600 rounded-full flex items-center justify-center opacity-0 animate-[stamp_0.5s_ease-out_0.5s_forwards] -rotate-12 pointer-events-none mix-blend-multiply">
                         <div className="text-yellow-600 font-black text-3xl uppercase text-center leading-none tracking-tighter">
                            Dinheiro<br/>Público
                         </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-3 w-full">
                    ...
                    <div className="flex gap-3 justify-center w-full">
                        <button 
                            ...
                            disabled={generating}
                            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-xl shadow-lg flex items-center justify-center gap-2 transition-transform active:scale-95 text-xs uppercase tracking-wider"
                        >
                        ...
                        <button 
                            onClick={onClose} 
                            className="bg-white text-gray-800 font-bold py-3 px-4 rounded-xl shadow-lg flex items-center justify-center gap-2 hover:bg-gray-100 transition-colors border border-gray-200"
                        >
                             <X size={20}/>
                        </button>
                    </div>
                    ...
                    {expense.link && (
                        <a 
                            ...
                            className="w-full py-3 bg-black/5 dark:bg-white/10 rounded-xl text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider flex items-center justify-center gap-2 hover:bg-black/10 dark:hover:bg-white/20 transition-colors"
                        >
                            Ver Documento Original <ExternalLink size={14}/>
                        </a>
                    )}
                </div>
            </div>
...
const TimelineCard: React.FC<{ item: TimelineItem; onOpenReceipt?: (item: TimelineItem) => void }> = ({ item, onOpenReceipt }) => {
    const isExpense = item.type === 'despesa';
    const docNumber = item.id.startsWith('exp-') ? item.id.replace('exp-', '') : null;

    const handleClick = () => {
        if (isExpense && onOpenReceipt) {
            onOpenReceipt(item);
        }
    };
    ...
    return (
        <div 
            className={`relative pl-8 pb-8 group ${isExpense ? 'cursor-pointer' : ''}`}
            onClick={handleClick}
        >
            ...
            <div className={`bg-white/70 dark:bg-gray-900/60 backdrop-blur-xl rounded-[2rem] border border-white/50 dark:border-gray-700 p-6 shadow-sm hover:shadow-xl transition-all duration-300 hover:scale-[1.01] relative ${isExpense ? 'hover:border-blue-200 dark:hover:border-blue-900' : ''}`}>
                
                <div className="flex justify-between items-start mb-3 pr-8">
                    <span className="text-[10px] font-bold text-blue-400 uppercase tracking-wider bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded-full flex items-center gap-1">
                    ...
                    {item.status && <span className={`text-[10px] font-bold uppercase px-3 py-1 rounded-full ${item.status === 'Sim' ? 'bg-blue-100 text-blue-700' : item.status === 'Não' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-blue-500'}`}>{item.status}</span>}
                    {item.type === 'evento' && (
                        <span className="text-[10px] font-bold uppercase bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 px-3 py-1 rounded-full">Agenda</span>
                    )}
                </div>
                
                <h4 className="font-bold text-blue-900 dark:text-white mb-2 leading-tight text-lg">{item.title}</h4>
                
                {item.description && (
                    <div className="text-sm text-blue-800 dark:text-gray-300 leading-relaxed mb-4 font-medium line-clamp-3">
                ...
                {item.link && (
                    <button 
                        ...
                        className="inline-flex items-center gap-2 text-xs font-bold text-white bg-black dark:bg-white dark:text-black px-4 py-2 rounded-full hover:opacity-80 transition-opacity"
                    >
                        {item.type === 'despesa' ? 'Ver Detalhes & Recibo' : 'Fonte Oficial'} <ExternalLink size={10}/>
                    </button>
                )}
            </div>
        </div>
...
const SpendingThermometer = ({ current, average }: { current: number, average: number }) => {
    ...
    const safeAvg = average || 1;
    const ratio = safeAvg > 0 ? current / safeAvg : 0;
    const barPercent = Math.min(Math.max((ratio * 50), 5), 100); 
    const isHigh = ratio > 1.2;
    const isLow = ratio < 0.8;
    const colorClass = isHigh ? 'bg-yellow-500' : isLow ? 'bg-blue-500' : 'bg-blue-300';
    const textClass = isHigh ? 'text-yellow-600' : isLow ? 'text-blue-500' : 'text-blue-300';
    ...
    return (
        <div className="w-full mt-4 bg-white/50 dark:bg-black/20 p-3 rounded-2xl border border-gray-100 dark:border-gray-700/50">
            <div className="flex justify-between items-end mb-2">
                <span className="text-[10px] font-bold text-blue-400 uppercase tracking-wider">Termômetro da Cota</span>
...
// --- CHART COMPONENT: ANNUAL SPENDING EVOLUTION ---
const AnnualSpendingBarChart = ({ stats, onSelectYear, activeYear }: { stats: Record<number, YearStats>, onSelectYear: (y: number | null) => void, activeYear: number | null }) => {
    const years = Object.keys(stats).map(Number).sort((a,b) => a - b);
    if (years.length === 0) return null;

    const data = years.map(y => ({ year: y, value: stats[y].spending }));
    const maxValue = Math.max(...data.map(d => d.value)) || 1;

    return (
        <div className="mt-4 w-full animate-in fade-in slide-in-from-bottom-2 duration-500">
            <p className="text-[10px] font-bold text-gray-400 uppercase mb-3 flex items-center justify-between">
                <span>Evolução Anual</span>
                <span className="text-[8px] bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">Clique para detalhar</span>
            </p>
            <div className="flex items-end gap-2 h-24 w-full">
                {data.map((item) => {
                    const heightPct = Math.max((item.value / maxValue) * 100, 5); // Min 5% height
                    const isActive = activeYear === item.year;
                    return (
                        <div
                            key={item.year}
                            onClick={(e) => { e.stopPropagation(); onSelectYear(isActive ? null : item.year); }}
                            className="flex-1 flex flex-col items-center group cursor-pointer"
                        >
                            <div className="relative w-full flex-1 flex items-end justify-center">
                                {/* Tooltip Hover */}
                                <div className="absolute bottom-full mb-1 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-900 text-white text-[9px] font-bold px-1.5 py-0.5 rounded pointer-events-none whitespace-nowrap z-20 shadow-xl">
                                    R$ {(item.value / 1000).toFixed(0)}k
                                </div>
                                <div
                                    className={`w-full max-w-[40px] rounded-t-lg transition-all duration-500 border-x border-t border-white/20 shadow-sm ${
                                        isActive 
                                        ? 'bg-gradient-to-t from-yellow-600 to-yellow-400' 
                                        : 'bg-gradient-to-t from-yellow-200 to-yellow-100 dark:from-yellow-900/40 dark:to-yellow-800/40 group-hover:to-yellow-300'
                                    }`}
                                    style={{ height: `${heightPct}%` }}
                                ></div>
                            </div>
                            <span className={`mt-2 text-[9px] font-bold transition-colors ${isActive ? 'text-yellow-600 dark:text-yellow-400 scale-110' : 'text-gray-400'}`}>
                                {item.year}
                            </span>
                        </div>
                    );
...
const ProfileView: React.FC<ProfileViewProps> = ({ candidate: initialCandidate, onBack, onCompare, onShare, feedItems, onUpdate, allPoliticians, isFollowing, onToggleFollow }) => {
  ...
  // Novo state para busca de votações
  ...
  const comparisonData = useMemo(() => {
      const FALLBACK_GENERAL = 38000; // Média estimada caso dados não estejam carregados
      
      if (!candidate || !allPoliticians) return { candidate: candidate.stats.spending, general: FALLBACK_GENERAL };
  ...
  const ContactCard = () => (
      <div className="bg-white/70 dark:bg-gray-800/60 backdrop-blur-xl rounded-[2.5rem] p-6 border border-white/50 dark:border-gray-700 shadow-sm space-y-4 h-full flex flex-col justify-between">
          <h3 className="font-black text-blue-900 dark:text-white text-lg flex items-center gap-2">
              <Mail size={20} className="text-blue-600"/> Contatos Oficiais
          </h3>
          
          <div className="flex gap-4 items-start p-3 bg-gray-50/50 dark:bg-black/20 rounded-[1.5rem]">
              <div className="p-2 bg-white dark:bg-gray-700 rounded-full text-blue-500 shadow-sm shrink-0">
                  <Building2 size={16}/>
              </div>
              <div className="min-w-0">
                  <p className="text-[10px] font-bold text-blue-400 uppercase tracking-wide">Gabinete</p>
                  <p className="text-xs font-bold text-blue-800 dark:text-white leading-tight mt-0.5 truncate">
          ...
          <div className="flex gap-2 items-center p-3 bg-gray-50/50 dark:bg-black/20 rounded-[1.5rem] group">
              <div className="p-2 bg-white dark:bg-gray-700 rounded-full text-blue-600 shadow-sm shrink-0">
                  <Phone size={16}/>
              </div>
              <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-bold text-blue-400 uppercase tracking-wide">Telefone</p>
                  <p className="text-xs font-bold text-blue-900 dark:text-white truncate">{candidate.cabinet?.phone || 'N/A'}</p>
              </div>
              {candidate.cabinet?.phone && (
                  <button 
                    onClick={() => handleCopy(candidate.cabinet?.phone || '', 'phone')}
                    className="p-2 bg-white dark:bg-gray-700 rounded-full hover:bg-blue-100 dark:hover:bg-blue-900/30 text-blue-400 hover:text-blue-600 transition-colors"
                  >
                      {copiedField === 'phone' ? <Check size={14} className="text-blue-600"/> : <Copy size={14}/>}
                  </button>
              )}
          </div>
          ...
          <div className="flex gap-2 items-center p-3 bg-gray-50/50 dark:bg-black/20 rounded-[1.5rem] group">
              ...
              <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-bold text-blue-400 uppercase tracking-wide">E-mail</p>
                  <a href={candidate.email ? `mailto:${candidate.email}` : '#'} className="text-xs font-bold text-blue-900 dark:text-white truncate hover:underline block">{candidate.email || 'N/A'}</a>
              </div>
              {candidate.email && (
                  <button 
                    ...
                    className="p-2 bg-white dark:bg-gray-700 rounded-full hover:bg-blue-100 dark:hover:bg-blue-900/30 text-blue-400 hover:text-blue-600 transition-colors"
                  >
                      {copiedField === 'email' ? <Check size={14} className="text-blue-600"/> : <Copy size={14}/>}
                  </button>
              )}
          </div>
          ...
          {candidate.socials && candidate.socials.length > 0 && (
              <div className="pt-2 border-t border-gray-100 dark:border-gray-700/50">
                  <p className="text-[10px] font-bold text-blue-400 uppercase tracking-wide mb-2">Redes Sociais</p>
                  <div className="flex flex-wrap gap-2">
                      {candidate.socials.slice(0, 4).map((url, idx) => {
                          const Icon = getSocialIcon(url);
                          return (
                              <a key={idx} href={url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 flex items-center justify-center rounded-full bg-white dark:bg-gray-700 text-blue-600 dark:text-gray-300 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-all shadow-sm">
                                  <Icon size={16} />
                              </a>
                          )
                      })}
    ...
    return (
        <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide mb-2">
            <button 
                ...
                className={`px-4 py-2 rounded-full text-xs font-bold transition-all whitespace-nowrap ${selectedYear === null ? 'bg-black text-white dark:bg-white dark:text-black shadow-lg' : 'bg-gray-100 text-blue-500 dark:bg-gray-800 dark:text-gray-400'}`}
            >
                Mandato Total
            </button>
            {years.map(year => (
                <button 
                    ...
                    className={`px-4 py-2 rounded-full text-xs font-bold transition-all whitespace-nowrap ${selectedYear === year ? 'bg-black text-white dark:bg-white dark:text-black shadow-lg' : 'bg-gray-100 text-blue-500 dark:bg-gray-800 dark:text-gray-400'}`}
                >
                    {year}
                </button>
            ))}
  ...
  const ActivityList = () => {
      ...
      if (loadingYearData) {
          return (
              <div className="py-8 text-center text-blue-400 flex flex-col items-center">
      ...
      // Filter Logic for Votes
      const filteredVotes = detailedVotes.filter(v => 
          v.description.toLowerCase().includes(voteSearchQuery.toLowerCase()) ||
          v.vote.toLowerCase().includes(voteSearchQuery.toLowerCase())
      );
          ...
          <div className="mt-6 border-t border-gray-100 dark:border-gray-700/50 pt-4">
              <div className="flex gap-2 mb-4 overflow-x-auto scrollbar-hide">
                  <button 
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-colors ${activityTab === 'votes' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : 'text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                  >
                      Votações
                  </button>
                  <button 
                    onClick={() => setActivityTab('relatorias')}
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-colors ${activityTab === 'relatorias' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : 'text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                  >
                      Relatorias
                  </button>
              </div>

              {/* SEARCH INPUT FOR VOTES */}
              {activityTab === 'votes' && (
                  <div className="relative mb-3">
                      ...
                      <input 
                          ...
                          className="w-full pl-9 pr-3 py-2 bg-gray-100 dark:bg-gray-800 rounded-xl text-xs font-bold text-gray-700 dark:text-gray-200 border-none focus:ring-2 focus:ring-blue-500/50 placeholder-gray-400"
                      />
                  </div>
              )}
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                  {activityTab === 'votes' && (
                      filteredVotes.length > 0 ? filteredVotes.map((v, i) => (
                          <div key={i} className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800/50">
                              <div className={`mt-0.5 p-1.5 rounded-full shrink-0 ${v.vote.toLowerCase().includes('sim') ? 'bg-blue-100 text-blue-900' : v.vote.toLowerCase().includes('não') || v.vote.toLowerCase().includes('nao') ? 'bg-yellow-100 text-yellow-600' : 'bg-gray-200 text-gray-500'}`}>
                              ...
                              <div>
                                  <p className="text-xs font-bold text-blue-800 dark:text-gray-200 line-clamp-2 leading-tight">{v.description}</p>
                                  <p className="text-[10px] text-blue-400 mt-1 font-medium">{new Date(v.date).toLocaleDateString('pt-BR')} • Voto: <span className="uppercase">{v.vote}</span></p>
                              </div>
                          </div>
                      )) : <p className="text-xs text-blue-400 text-center py-4 font-medium">Nenhuma votação encontrada.</p>
                  )}
                  ...
                  {activityTab === 'relatorias' && (
                      detailedRelatorias.length > 0 ? detailedRelatorias.map((r, i) => (
                          <div key={i} className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800/50">
                              <div className="flex justify-between mb-1">
                                  <span className="text-[10px] font-black bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300 px-2 py-0.5 rounded uppercase">{r.billType}</span>
                                  <span className="text-[10px] text-blue-400 font-bold">{new Date(r.date).toLocaleDateString('pt-BR')}</span>
                              </div>
                              <p className="text-xs font-bold text-blue-800 dark:text-gray-200 mb-2 leading-tight">{r.billTitle}</p>
                      ...
                      )) : <p className="text-xs text-blue-400 text-center py-4 font-medium">Nenhuma relatoria neste ano.</p>
                  )}
              </div>
          </div>
      );
  ...
  const StatsCard = () => {
      ...
      if (candidate.role === 'Senador') {
          ...
          return (
             <div className="bg-white/70 dark:bg-gray-800/60 backdrop-blur-xl rounded-[2.5rem] p-6 border border-white/50 dark:border-gray-700 shadow-sm h-full flex flex-col justify-between">
                <div>
                    <div className="flex items-center gap-2 mb-4">
                        <Database className="text-blue-500" size={20}/>
                        <h4 className="font-bold text-blue-800 dark:text-white">Atividade no Senado</h4>
                    </div>
                    ...
                    {hasVotes || hasSpeeches ? (
                            ...
                            {hasVotes && (
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-[1.5rem] border border-blue-100 dark:border-blue-800/30 flex justify-between items-center">
                                    <div>
                                        ...
                                        <p className="text-2xl font-black text-blue-900 dark:text-white">{candidate.stats.projects}</p>
                                    </div>
                                </div>
                            )}
                            {hasSpeeches && (
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-[1.5rem] border border-blue-100 dark:border-blue-800/30 flex justify-between items-center">
                                    <div>
                                        <p className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wide mb-1">Discursos</p>
                                        <p className="text-2xl font-black text-blue-900 dark:text-white">{candidate.speeches?.length || 0}</p>
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="py-4 text-center">
                            <p className="text-xs text-blue-500 font-medium">Dados de atividade detalhada não disponíveis via API pública neste momento.</p>
                        </div>
                    )}
                </div>
                ...
                <button 
                    ...
                    className="mt-4 w-full py-3 bg-gradient-to-r from-blue-600 to-blue-800 rounded-2xl text-white font-bold text-xs uppercase tracking-wider shadow-lg flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform"
                >
      ...
      // DEPUTADOS
      return (
        <div className="bg-white/70 dark:bg-gray-800/60 backdrop-blur-xl rounded-[2.5rem] p-6 border border-white/50 dark:border-gray-700 shadow-sm h-full flex flex-col">
            <div className="flex justify-between items-center mb-3">
                <h4 className="font-bold text-blue-900 dark:text-white text-sm">Atividades na Câmara</h4>
                <button 
                    onClick={handleAiAnalysis}
                    className="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center gap-1 hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors"
                >
            ...
            <div className="grid grid-cols-1 gap-4 flex-1">
                <div className="flex gap-4">
                    <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-[1.5rem] border border-blue-100 dark:border-blue-800/30 flex-1">
                        <p className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wide mb-1">Presença</p>
                        <p className="text-xl font-black text-blue-900 dark:text-white">
                            {displayedStats.attendancePct}%
                            {displayedStats.attendancePct === 0 && candidate.hasApiIntegration && <span className="text-[8px] block text-blue-400 font-normal">Calculando...</span>}
                        </p>
                    </div>
                    <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-[1.5rem] border border-blue-100 dark:border-blue-800/30 flex-1">
                        <p className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wide mb-1">Projetos</p>
                        <p className="text-xl font-black text-blue-900 dark:text-white">{displayedStats.projects}</p>
                    </div>
                </div>
                
                {/* TERMÔMETRO DE GASTOS & CHART ANUAL */}
                <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-[1.5rem] border border-yellow-100 dark:border-yellow-800/30">
                    <div className="flex justify-between items-center">
                        ...
                        <p className="text-lg font-black text-blue-900 dark:text-white">R$ {Math.round(displayedStats.spending).toLocaleString('pt-BR')}</p>
                    </div>
                    
                    {/* CHART DE EVOLUÇÃO ANUAL (Apenas no Overview) */}
                    {!selectedYear && candidate.yearlyStats && Object.keys(candidate.yearlyStats).length > 0 && (
                        <AnnualSpendingBarChart 
                            stats={candidate.yearlyStats} 
                            onSelectYear={setSelectedYear} 
                            activeYear={selectedYear} 
                        />
                    )}

                    {/* Termômetro (Apenas se não houver gráfico de anos ou se for um ano específico) */}
                    {(!candidate.yearlyStats || Object.keys(candidate.yearlyStats).length === 0 || selectedYear) && displayedStats.spending > 0 && (
                        <SpendingThermometer 
                    ...
                    {!selectedYear && displayedStats.spending === 0 && candidate.hasApiIntegration && (
                         <div className="mt-2 text-xs text-blue-500 flex items-center gap-2">
                             <Loader2 size={12} className="animate-spin"/> Processando dados financeiros...
                         </div>
                    )}
                </div>
            </div>
  ...
  const TabButton = ({ id, label, count }: { id: typeof profileTab, label: string, count?: number }) => (
      <button 
        ...
        className={`px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all duration-300 flex items-center gap-2 relative overflow-hidden group
            ${profileTab === id ? 'text-white shadow-lg' : 'text-blue-500 hover:text-blue-900 dark:text-gray-400 dark:hover:text-white hover:bg-white/40 dark:hover:bg-white/10'}`}
      >
        {profileTab === id && (
        ...
        {count !== undefined && (
            <span className={`relative z-10 text-[10px] py-0.5 px-2 rounded-full transition-colors ${profileTab === id ? 'bg-white/20 text-white dark:bg-black/10 dark:text-black' : 'bg-gray-200 dark:bg-gray-700 text-blue-600 dark:text-gray-300'}`}>
                {count}
            </span>
        )}
      </button>
  ...
  return (
    <div className="w-full h-full overflow-y-auto bg-transparent animate-in slide-in-from-right duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] scrollbar-hide pb-32">
      ...
      {showAiSearch && (
          <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in" onClick={() => setShowAiSearch(false)}>
              <div className="bg-white dark:bg-gray-900 w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[85vh] relative" onClick={e => e.stopPropagation()}>
                  <div className="p-6 bg-gradient-to-r from-blue-600 to-blue-800 flex justify-between items-center text-white shrink-0">
                  ...
                  <div className="p-8 overflow-y-auto flex-1">
                      {aiLoading ? (
                          <div className="flex flex-col items-center justify-center py-12 text-center">
                              ...
                              <p className="text-blue-500 font-bold">Pesquisando e analisando dados recentes...</p>
                              <p className="text-xs text-blue-400 mt-2">Isso pode levar alguns segundos.</p>
                          </div>
                      ) : aiData ? (
                          <div className="space-y-6">
                              <div className="prose dark:prose-invert prose-sm max-w-none">
                                  <p className="text-lg font-medium leading-relaxed whitespace-pre-line text-blue-700 dark:text-gray-200">
                                      {aiData.text}
                                  </p>
                              </div>
                              ...
                              {aiData.sources && aiData.sources.length > 0 && (
                                  <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
                                      <h4 className="text-xs font-black uppercase text-blue-400 mb-3 flex items-center gap-2">
                  ...
                  <div className="p-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-800 text-center text-[10px] text-blue-400 font-medium">
      ...
      <div className="relative w-full md:w-[calc(100%-2rem)] mx-auto mt-2 md:mt-4 overflow-hidden rounded-[3rem] shadow-xl bg-gray-900 min-h-[220px] md:min-h-[270px] flex items-end">
          <div className="absolute inset-0 bg-gradient-to-br from-blue-700 via-blue-500 to-blue-900"></div>
          ...
          <div className="absolute top-4 left-4 right-4 z-50 bg-white/10 backdrop-blur-md border border-white/20 p-2.5 rounded-full flex items-center justify-between shadow-lg">
            ...
            <div className="flex gap-2">
                 ...
                 <button onClick={onShare} className="px-4 py-2.5 bg-blue-600 text-white hover:bg-blue-700 rounded-full transition-colors active:scale-95 text-xs font-bold flex items-center gap-2 border border-blue-500 shadow-lg">
      ...
      <div className="px-4 md:px-8 pb-32 max-w-[1800px] mx-auto mt-8 relative z-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
               ...
               <div className="bg-white/70 dark:bg-gray-800/60 backdrop-blur-xl rounded-[2.5rem] p-6 border border-white/50 dark:border-gray-700 shadow-sm space-y-5 flex flex-col">
                   <h3 className="font-black text-blue-900 dark:text-white text-lg border-b border-gray-100 dark:border-gray-700/50 pb-3">Sobre</h3>
                   
                   <div className="space-y-4 flex-1">
                       <div>
                           <p className="text-[10px] text-blue-400 font-bold uppercase">Nome Civil</p>
                           <p className="text-sm font-bold text-blue-800 dark:text-white">{candidate.civilName || candidate.name}</p>
                       </div>
                       
                       <div className="grid grid-cols-2 gap-4">
                           <div>
                               <p className="text-[10px] text-blue-400 font-bold uppercase">Naturalidade</p>
                               <p className="text-sm font-bold text-blue-800 dark:text-white truncate">{candidate.birthCity ? `${candidate.birthCity} - ${candidate.birthState}` : 'Não informado'}</p>
                           </div>
                           <div>
                                <p className="text-[10px] text-blue-400 font-bold uppercase">Nascimento</p>
                                <p className="text-sm font-bold text-blue-800 dark:text-white">{candidate.birthDate || 'N/A'}</p>
                           </div>
                       </div>
                       
                       <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-blue-100 dark:bg-gray-700 flex items-center justify-center text-blue-500 shrink-0"><GraduationCap size={18} /></div>
                            <div><p className="text-[10px] text-blue-400 font-bold uppercase">Escolaridade</p><p className="text-sm font-bold text-blue-800 dark:text-white">{candidate.education || 'Não informado'}</p></div>
                       </div>

                       {/* NOVO: CARREIRA (Histórico Ocupacional) */}
                       {candidate.occupations && candidate.occupations.length > 0 && (
                           <div className="pt-2 border-t border-gray-100 dark:border-gray-700/50">
                               <p className="text-[10px] text-blue-400 font-bold uppercase mb-2">Carreira Prévia</p>
                               <div className="flex flex-wrap gap-2">
                                   {candidate.occupations.slice(0, 3).map((job, i) => (
                                       <span key={i} className="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded text-[10px] font-bold text-blue-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600">
                                           <BriefcaseBusiness size={10}/> {job.title}
                                       </span>
                                   ))}
                                   {candidate.occupations.length > 3 && <span className="text-[10px] text-blue-400 font-bold">+{candidate.occupations.length - 3}</span>}
                               </div>
                           </div>
                       )}
                       ...
                       {candidate.suplentes && candidate.suplentes.length > 0 && (
                           <div className="pt-2 border-t border-gray-100 dark:border-gray-700/50">
                               <p className="text-[10px] text-blue-400 font-bold uppercase mb-2">Suplentes (Em caso de licença)</p>
                               <div className="flex flex-col gap-1">
                                   {candidate.suplentes.map((sup, i) => (
                                       <div key={i} className="flex items-center gap-2 text-xs font-bold text-blue-600 dark:text-gray-300">
          ...
          <div className="min-w-0">
              ...
              <div className="space-y-6 min-h-[50vh]">
                          ...
                          {candidate.timeline && candidate.timeline.length > 0 ? (
                              <div className="relative">
                                  <div className="space-y-0 pb-10">
                                      {candidate.timeline.map((item) => <TimelineCard key={item.id} item={item} onOpenReceipt={setSelectedReceipt} />)}
                                  </div>
                              ...
                              <div className="text-center py-20 text-blue-400 bg-white/50 dark:bg-gray-800/50 rounded-[2.5rem] border border-dashed border-gray-200 dark:border-gray-700">
                                  ...
                                  <p className="text-xs mt-1 opacity-70 max-w-[250px] mx-auto">
                                      {candidate.hasApiIntegration 
                                          ? 'Ainda não há dados recentes processados para este perfil.' 
                       ...
                       <div className="animate-in fade-in slide-in-from-bottom-8 duration-500">
                               ...
                               <div className="bg-white dark:bg-gray-800 p-6 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-sm mb-6 flex flex-col md:flex-row items-center gap-8">
                                   <div className="relative w-48 h-48 shrink-0">
                                        ...
                                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                            {activeExpense ? (
                                                <>
                                                    <span className="text-2xl font-black text-blue-900 dark:text-white">{activeExpense.percent}%</span>
                                                    <span className="text-[10px] font-bold text-blue-400 uppercase text-center max-w-[80px] truncate">{activeExpense.type}</span>
                                                </>
                                            ) : (
                                                <span className="text-xs font-bold text-blue-400 uppercase">Gastos</span>
                                            )}
                                        </div>
                                   </div>
                                   <div className="flex-1 grid grid-cols-2 gap-3 w-full">
                                       {chartSegments.slice(0, 6).map((seg, i) => (
                                           <div key={i} className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" onMouseEnter={() => setHoveredExpenseIndex(i)} onMouseLeave={() => setHoveredExpenseIndex(null)}>
                                               ...
                                               <div className="min-w-0">
                                                   <p className="text-xs font-bold text-blue-700 dark:text-gray-300 truncate">{seg.type}</p>
                                                   <p className="text-[10px] text-blue-400">R$ {seg.value.toLocaleString('pt-BR')}</p>
                                               </div>
                                           </div>
                                       ))}
                                   </div>
                                ...
                                <div className="text-center py-20 text-blue-400">
                                    ...
                                    <p className="font-bold text-sm">Dados financeiros indisponíveis no momento.</p>
                                </div>
                           )}
                           ...
                           {candidate.expensesHistory && candidate.expensesHistory.length > 0 && (
                               <div className="bg-white dark:bg-gray-800 p-6 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-sm mb-6">
                                   <h4 className="text-xs font-black uppercase text-blue-400 mb-4">Evolução de Gastos</h4>
                           ...
                           {topSuppliers.length > 0 && (
                               <div className="bg-white dark:bg-gray-800 p-6 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-sm">
                                   <h4 className="text-xs font-black uppercase text-blue-400 mb-4">Maiores Fornecedores</h4>
                                   <div className="space-y-3">
                                       {topSuppliers.map((sup, i) => (
                                           <div key={i} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                                               <span className="text-xs font-bold text-blue-700 dark:text-gray-200 truncate flex-1 pr-4">{sup.name}</span>
                                               <span className="text-xs font-black text-blue-900 dark:text-white whitespace-nowrap">R$ {sup.val.toLocaleString('pt-BR')}</span>
                                           </div>
                                       ))}
                                   </div>
                               </div>
                  ...
                  {/* REST OF TABS (PROJECTS, ROLES, ETC) - KEEPING EXISTING LOGIC BUT ENSURING SKELETONS */}
                  {profileTab === 'roles' && (
                                      ...
                                      <div>
                                          <div className="flex justify-between items-start mb-3">
                                              <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full flex items-center gap-1 ${
                                                  ...
                                                  'bg-gray-100 text-blue-600 dark:bg-gray-700 dark:text-gray-300'
                                              }`}>
                                                  {isPresidente && <Star size={10} className="fill-current"/>}
                                                  {role.title}
                                              </span>
                                              <span className="text-[10px] font-bold text-blue-400">{role.acronym}</span>
                                          </div>
                                          <h4 className="font-bold text-blue-900 dark:text-white text-sm leading-tight mb-2">{role.name}</h4>
                                          <p className="text-xs text-blue-500 font-medium">{role.type}</p>
                                      </div>
                                      <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700/50 flex items-center gap-2 text-[10px] font-bold text-blue-400 uppercase tracking-wider">
                                          <Calendar size={12}/> Desde {new Date(role.startDate).toLocaleDateString('pt-BR')}
                                      </div>
                                  </div>
                              );
                          }) : (
                              <div className="col-span-full text-center py-20 text-blue-400">
                           ...
                           {candidate.bills && candidate.bills.length > 0 ? candidate.bills.map((bill, i) => (
                               <div key={i} className="bg-white dark:bg-gray-800 p-6 rounded-[2rem] border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
                                   ...
                                   <div className="flex justify-between items-start mb-2 pl-4">
                                       ...
                                       <span className="text-[10px] font-bold text-blue-400">{new Date(bill.date).toLocaleDateString('pt-BR')}</span>
                                   </div>
                                   <div className="pl-4">
                                       <h4 className="font-black text-blue-900 dark:text-white text-base mb-1 group-hover:text-blue-600 transition-colors">
                                            {bill.title}
                                       </h4>
                                       <p className="text-sm text-blue-600 dark:text-gray-300 leading-relaxed line-clamp-2 mb-4">
                                           {bill.description}
                                       </p>
                                       {bill.externalLink && (
                              ...
                              {candidate.speeches && candidate.speeches.length > 0 ? candidate.speeches.map((speech, i) => (
                                  <div key={i} className="bg-white dark:bg-gray-800 p-6 rounded-[2rem] border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all">
                                      <div className="flex items-center gap-3 mb-3">
                                          ...
                                          <div>
                                              <p className="text-[10px] font-bold text-blue-400 uppercase">{new Date(speech.date).toLocaleDateString('pt-BR')}</p>
                                              <p className="text-xs font-black text-blue-600 dark:text-blue-400 uppercase">{speech.type}</p>
                                          </div>
                                      </div>
                                      <p className="text-sm text-blue-700 dark:text-gray-200 font-medium leading-relaxed italic">
                                          "{speech.summary}"
                                      </p>
                                      {speech.externalLink && (
                                           <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700/50">
                                               <a href={speech.externalLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-xs font-bold text-blue-500 hover:text-black dark:hover:text-white transition-colors">
                          ...
                          {candidate.speeches && candidate.speeches.length >= 5 && (
                              <div className="mt-6 text-center">
                                  <button 
                                      ...
                                      className="px-6 py-3 bg-gray-100 dark:bg-gray-800 rounded-full text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors disabled:opacity-50 flex items-center gap-2 mx-auto"
                                  >
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {candidate.fronts && candidate.fronts.length > 0 ? candidate.fronts.map((front, i) => (
                                  <a key={i} href={front.externalLink} target="_blank" rel="noopener noreferrer" className="bg-white dark:bg-gray-800 p-5 rounded-[2rem] border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all group flex items-start gap-4 hover:border-blue-200">
                                      ...
                                      <div className="flex-1 min-w-0">
                                          <h4 className="font-bold text-blue-900 dark:text-white text-sm leading-tight mb-1 group-hover:text-blue-600 transition-colors">{front.title}</h4>
                                          <p className="text-[10px] text-blue-400 font-bold uppercase">Frente Parlamentar</p>
                                      </div>
                          ...
                          {candidate.assets && candidate.assets.length > 0 ? (
                               <div className="space-y-4">
                                   <div className="flex justify-between items-center px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-xl">
                                       <span className="text-xs font-black uppercase text-blue-500">Total Declarado</span>
                                   ...
                                   {candidate.assets.map((asset, i) => (
                                       <div key={i} className="flex justify-between items-center p-4 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl">
                                           <div>
                                               <p className="text-sm font-bold text-blue-800 dark:text-white">{asset.description}</p>
                                               <p className="text-[10px] font-bold text-blue-400 uppercase">{asset.type}</p>
                                           </div>
                                           <span className="text-sm font-black text-blue-900 dark:text-white">{asset.value}</span>
                                       </div>
                                   ))}
                               </div>
                          ) : (
