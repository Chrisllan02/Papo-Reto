...
**Atualização (mesmo PR):** os itens marcados com ✅ abaixo já foram aplicados nesta branch. Lint caiu de 139 para 20 warnings (as 20 restantes são padrões pré-existentes de `react-hooks` — `set-state-in-effect`/`exhaustive-deps` — que exigem refatoração mais profunda de efeitos e não foram tocadas para não arriscar regressão de comportamento sem testes de UI que cubram esses fluxos).
...
| Item | Situação | Ação |
| --- | --- | --- |
| CSP | Ausente | ✅ Adicionado `Content-Security-Policy` no `vercel.json` |
| HSTS | Ausente | ✅ `Strict-Transport-Security: max-age=63072000; includeSubDomains` |
| Referrer-Policy / Permissions-Policy | Ausentes | ✅ `strict-origin-when-cross-origin`; câmera/pagamento negados |
| `X-XSS-Protection` | Presente, **deprecated** | ✅ Removido (substituído por CSP) |
| SSRF no `/api/camara` | **Bem protegido** (allowlist de hosts + https + limite de bytes + timeout) | — (sem ação necessária) |
| XSS | Nenhum uso de `dangerouslySetInnerHTML`/`innerHTML` | — (sem ação necessária) |
| Rate limiting | Em memória, por instância, `Map` sem limpeza (leak lento) | ✅ Limpeza de entradas expiradas adicionada; segue best-effort por instância (aceito, ver nota) |
| Node engines | `>=18` (EOL desde abr/2025); CI e Vercel usam 20 | ✅ Subido para `>=20` |

> Nota sobre rate limiting: mover para um store persistente (Upstash/KV) exigiria nova infraestrutura paga fora do escopo desta varredura; a limpeza de entradas expiradas evita memory leak, e a autenticação por origem (itens 1.2/1.3) já reduz a superfície de abuso mesmo com rate limit por instância.
>
> URLs de produção hardcoded em `legislativeBootstrap.ts`/cron (`papo-reto-beige.vercel.app`) foram mantidas — mudar exigiria testar contra `VERCEL_URL` em preview deployments reais, fora do alcance de verificação local desta sessão.
...
> Não aplicado nesta rodada: é uma refatoração estrutural grande, de risco desproporcional ao benefício sem uma suíte de testes de UI que garanta que nada quebrou. `DnaDonutChart` e `QuickMetric` já foram extraídos de `ProfileView` como parte da correção do item 4.5, iniciando esse trabalho.
...
> Não aplicado: exige mudar o contrato de resposta de `/api/bootstrap` e validar contra a API real da Câmara, fora do alcance de verificação local desta sessão (rede externa bloqueada no ambiente de execução).
...
> Não aplicados nesta rodada — todos exigem integração de dados nova (endpoints do Senado, `/votacoes/{id}/votos`) validada contra a API real, que está fora do alcance desta sessão (o ambiente de execução bloqueia rede externa; confirmado via smoke test que a rede está de fato indisponível aqui). Ficam registrados como próximos passos priorizados na seção 7.
...
- **139 warnings de lint** — ✅ reduzido para 20 (as 20 restantes são padrões pré-existentes de `react-hooks` que exigem refatoração de efeitos fora do escopo seguro desta rodada; ver nota no topo do documento).
- **`alert()`** usado para erro de geolocalização (`AppContext`) — ✅ trocado por `console.warn`, consistente com o resto do tratamento de erros silencioso do app.
- **Cobertura de testes desequilibrada**: os 31 testes cobrem bem os handlers serverless e utils, mas só 1 componente (`DataState`). Zero testes das views principais, do `AppContext` (navegação/tema/local) e dos hooks de dados. `playwright` já está em devDependencies e não há nenhum E2E — um smoke E2E (abrir mural → abrir perfil → voltar) pegaria regressões que os testes atuais não veem. *(Não aplicado: adicionar suíte de testes é esforço grande e não fazia parte do pedido original de correção; validei manualmente as mudanças de rotas com Playwright nesta sessão, ver seção de verificação.)*
- **Histórico do git poluído**: os 10+ commits mais recentes de `main` são `Update README dynamic assets` do bot diário. *(Não aplicado: mudar o workflow de CI é uma decisão de processo que cabe ao time, não uma correção técnica.)*
- **Sem CONTRIBUTING/CHANGELOG/licença** definidos no repositório. *(Não aplicado: decisão de produto/organização, não bug técnico.)*

## 7. Oportunidades de produto

1. ✅ **Rotas + compartilhamento de perfil** (4.1/4.2): perfis agora são linkáveis (`/politico/:id`), com meta description, Open Graph/Twitter Card e imagem de preview.
2. ✅ **PWA**: manifest (`public/manifest.webmanifest`) + service worker (`public/sw.js`, cache do shell estático) adicionados.
3. **Paridade do Senado** (item 5) — dobra o valor do produto. *(Não aplicado — depende de rede externa indisponível neste ambiente.)*
4. **Busca global** (parlamentar, proposição, tema) — hoje a busca existe só dentro de Explore. *(Não aplicado — feature nova, fora do escopo de correção de gaps existentes.)*
5. ✅ **Acompanhar/seguir parlamentar**: implementado com persistência em `localStorage` (`toggleFollow`/`followedIds` no `AppContext`) e botão "Acompanhar" no cabeçalho do perfil.
6. **Votações por parlamentar** (item 5, `votingHistory`) — "como votou meu deputado" é a pergunta nº 1 de transparência. *(Não aplicado — depende de rede externa indisponível neste ambiente.)*
7. **Comparador de parlamentares/partidos** (despesas, presença, produção) — os dados já estão no cache. *(Não aplicado — feature nova, fora do escopo de correção de gaps existentes.)*
...
| --- | --- | --- | --- | --- |
| 1 | `npm audit fix` (undici) antes do próximo push | XS | CI verde | ✅ Feito |
| 2 | Proteger `/api/ai` e `/api/profile-cache` (origem) | S | Custo/abuso | ✅ Feito |
| 3 | Corrigir cache de falha de 24h (2.1) e fluxo morto do profile-cache (2.2) | S | Dados/performance | ✅ Feito |
| 4 | Headers de segurança + viewport com zoom | XS | Segurança/A11y | ✅ Feito |
| 5 | Rotas por URL + meta tags/OG | M | Alcance do produto | ✅ Feito |
| 6 | Error Boundary + zerar warnings de lint | S | Estabilidade | ✅ Feito (139→20 warnings; as 20 restantes são pré-existentes e documentadas) |
| 7 | Perfis do Senado + votingHistory | M/L | Valor do produto | Pendente — depende de rede externa indisponível neste ambiente de execução |
| 8 | PWA + follow de parlamentar | M | Retenção | ✅ Feito |

### Verificação desta rodada
`npm run lint`, `npm test` (31/31) e `npm run build` passam. Testado manualmente com Playwright contra `vite preview`: carregamento do app, navegação entre abas atualizando a URL, deep link direto (`/partidos`, `/politico/:id` inválido → sem 404), e botão voltar/avançar do navegador — todos funcionando sem erros de console. Chamadas às APIs públicas (Câmara/Senado) não puderam ser validadas ao vivo pois este ambiente de execução bloqueia rede externa (`ERR_TUNNEL_CONNECTION_FAILED`); o app cai corretamente no fallback estático (`POLITICIANS_DB` vazio, por design — sem dados fabricados) nesse cenário.
